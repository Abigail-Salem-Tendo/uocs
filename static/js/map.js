...
function markerColorFor(report) {
    ...
    return STATUS_COLORS[normalizeStatus(report.status)] || "#757575";
}
...
async function renderOutageMap(elementId, dataUrl, options = {}) {
    ...
    reports.forEach((report) => {
        ...
        marker.addListener("click", () => {
            const statusLabel = normalizeStatus(report.status);
            infoWindow.setContent(`
        <strong>${report.type}</strong><br>
        Status: ${statusLabel}${report.isHotspot ? " (hotspot)" : ""}<br>
